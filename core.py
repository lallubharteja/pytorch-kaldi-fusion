...
def run_nn_refac02(
    data_name, data_set, data_end_index, fea_dict, lab_dict, arch_dict, cfg_file, processed_first, next_config_file, lab_vec_dict=None
):
    ...
    if lab_vec_dict is None:
        ...
        p = threading.Thread(target=read_lab_fea, args=(next_config_file, is_production, shared_list, output_folder), kwargs={'lab_vec_dict':lab_vec_dict})
    p.start()
    ...
    for i in range(N_batches):
        ...
        if to_do == "train":
            ...
            #for net in nns.keys():
            #    torch.nn.utils.clip_grad_norm_(nns[net].parameters(), 5)
