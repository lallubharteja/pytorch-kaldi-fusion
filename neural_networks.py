...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, n_head, d_model, d_k, d_v, dropout=0.1, add_residual=False):
        super().__init__()
...
class AttentionLayer(nn.Module):
    ''' Attend to the input layers  '''

    def __init__(self, options, inp_dim):
        super(AttentionLayer, self).__init__()
        self.input_dim = inp_dim
        self.n_comb = int(options["att_n_comb"])
        self.n_head = int(options["att_n_head"])
        self.d_k = int(options["att_key_dim"])
        self.d_v = int(options["att_value_dim"])
        self.dropout = float(options["att_dropout"])
        self.add_residual = strtobool(options["att_add_residual"])
        
        self.slf_attn = MultiHeadAttention(self.n_head, self.input_dim, self.d_k, self.d_v, self.dropout, self.add_residual)
